# Material Simulations Research Documentation

Welcome to the documentation for the Material Simulations Research project. This documentation provides comprehensive information about the research, methodology, models, and codebase.

## Overview

This research project investigates the dynamics of 2D mass-spring lattice systems using exponential spring models. The work is conducted in collaboration with Dr. Romesh Batra, Distinguished Professor and Clifton C. Garvin Professor, Department of Mechanical Engineering, Virginia Tech.

## Documentation Structure

### Getting Started
- [Installation and Setup](getting-started.md) - How to set up the development environment and run simulations

### Research Documentation
- [Methodology](methodology.md) - Research methodology, theoretical background, and approach
- [Models](models.md) - Detailed descriptions of the mathematical models used
- [Results](results.md) - Results, analysis, and findings (work in progress)
- [Optimization](optimization.md) - Material ordering optimization framework

### Technical Documentation
- [API Reference](api-reference.md) - Code documentation and function references

## Quick Links

- [Main README](../README.md) - Project overview and quick start
- [Source Code](../src/) - Simulation code
- [Optimization Scripts](../scripts/optimization/) - Material ordering optimization
- [Papers](../papers/) - LaTeX papers and research updates

## Project Status

⚠️ **Work in Progress**: This project is actively under development. Documentation and code are being refined as research progresses.

## Research Team

- **Researcher**: Daniel Miles (Virginia Tech Student)
- **Advisor**: Dr. Romesh Batra, Department of Mechanical Engineering, Virginia Tech

## Contact

For questions about this research or documentation, please contact:
- Daniel Miles: danielmiles@vt.edu


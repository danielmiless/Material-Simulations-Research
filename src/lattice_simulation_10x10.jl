"""
Exponential Spring Mass-Spring Lattice Simulation: 10×10 System with Immovable Backplate and Material Scaling

This script implements a 2D mass-spring lattice system (10×10) with exponential spring models,
an immovable backplate to the right of the lattice, and column-based material scaling.
The system includes:
...
- Column-based material property scaling (columns 2-10 scaled by MATERIAL_MULTIPLIER)
...
const F_MAG       = 1200.0        # N, magnitude of external force - increased for 10×10 to showcase material gradient
...
#  LATTICE SIZE  
########################################################################
const N = 10                     # ⇒ 10×10 = 100 masses
const TOTAL_MASSES = N * N       # 100 masses total
const DOF_PER_MASS = 2          # x and y components
const TOTAL_DOF = TOTAL_MASSES * DOF_PER_MASS  # 200 position DOF
...
########################################################################
@inline lattice_idx(i,j) = (i-1)*N + j                    # (i,j) → mass index 1…100
...
function get_column_k_coupling(j, material_multiplier)
    ...
    Column 1 uses base value, columns 2-10 use scaled values.
    """
    if j == 1
        return K_COUPLING
    else
...
function run_2d_simulation_with_configurable_force_and_backplate()
    ...
    """
    println("10×10 Lattice Mass-Spring System with Configurable Force, Immovable Backplate, and Material Scaling")
    println("=" ^ 80)
    ...
    println("  ... (columns 5-9 with intermediate scaling) ...")
    for j in N:N
        k_col = get_column_k_coupling(j, MATERIAL_MULTIPLIER)
        c_col = get_column_c_damping(j, MATERIAL_MULTIPLIER)
        println("  Column $j: $(round(MATERIAL_MULTIPLIER^(j-1), digits=3))× base (K=$(round(k_col, digits=1)) N, c=$(round(c_col, digits=2)) N·s/m)")
    ...
    # Main animation axis
    ax = Axis(fig[1, 1:2], 
              title = "10×10 Mass-Spring Lattice with Configurable Force, Immovable Backplate, and Material Scaling",
              xlabel = "X Position",
              ylabel = "Y Position",
              aspect = DataAspect())
    ...
    # Color scheme based on column index to visualize material gradient (10 columns)
    column_colors = [:lightblue, :lightcyan, :cyan, :blue, :mediumblue, :darkblue, :navy, :midnightblue, :darkslateblue, :black]
    for k in 1:TOTAL_MASSES
        i, j = lattice_i(k), lattice_j(k)
        if k == target_idx
            colors[k] = :orange  # Target mass always orange
        else
            # Map column index to color (1-based, so j=1..10)
    ...
    stats_text = @lift begin
        ...
        - Material scaling: $(MATERIAL_MULTIPLIER)× multiplier (columns 2-10 scaled)
...
########################################################################
#  OPTIONAL: SAVE ANIMATION TO FILE WITH FORCE CONFIGURATION AND BACKPLATE
########################################################################
function save_animation_to_file(filename = "lattice_anim_with_backplate.mp4")
    """
    Save the animation to a video file with force configuration display and backplate.
    """
...
save_animation_to_file("animations/lattice_anim_10x10_with_backplate.mp4");

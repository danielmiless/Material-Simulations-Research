...
\begin{abstract}
\noindent We use a spring-mass system to analyze transient deformations of a sandwich plate impact loaded on the top surface. It is assumed that each layer can be simulated as an assembly of an 11×11 spring-mass-dashpot system. We develop and analyze an 11×11 two-dimensional spring-mass-dashpot network (121 masses, 242 degrees of freedom) featuring springs with exponential force--stretch relations, viscous damping, and an immovable backplate constraint. The system is subjected to distributed external loading, and the dynamic response is solved using higher-order numerical integration methods. Energy balance verification is used to confirm numerical solution quality, with work--energy relationships accounting for energy dissipation through damping. We then study how the ordering of core layers affects the peak force transferred to the back surface, motivated by the need to optimize protective sandwich-plate configurations. We formulate the layer ordering as a permutation optimization problem and implement six optimization algorithms in Julia (Metaheuristics.jl, BlackBoxOptim.jl, and Optim.jl). With 100--102 function evaluations (each requiring a full transient simulation), we obtain configurations that reduce the peak transmitted force relative to simple baseline orderings. Expected end-of-semester outcomes include a verified spring-mass-dashpot model for a representative impact loading scenario, a documented optimization workflow for layer ordering, and a consolidated report describing the governing equations, verification, and optimization results.
\end{abstract}
